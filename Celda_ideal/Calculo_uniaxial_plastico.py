...
try:
    for j in range(0,len(direcciones)):
        ...
        working_dir = os.path.join(temporal, f"sim_{dir}_plast") # Create a temporary directory to store and delete obsolete files
        ...
        ##                          INITIALISATION                          ##
        ######################################################################
        file_results = rf"{reports_dir_cell}\{file_name}_plastic_results_{structure}x{structure}x{structure}_{dir}"
        ...
        ######################################################################
        ##            /SOLU |SOLVER| AND /POST1 |POSTPROCESS |              ##
        ######################################################################
        if dir == "X":
            A = A_x # [m^2]
            dist = dist_x # characteristic length
        if dir == "Y":
            A = A_y # [m^2]
            dist = dist_y # characteristic length
        if dir == "Z":
            A = A_z # [m^2]
        ...
        while  rotura == False:
            ...
            for i in range(1, new_substp+1):
                ...
                EZ, s, e, nu, f = prop_mec(mapdl, delta, dist, A, dir)
        ...
        ax.plot(disp_global, FR/1e3, color='blue', linewidth=2, label='Force-Displacement') # FR [kN] and displacmenet [mm] 
        ...
        # Labels and titles
        ax.set_title(f'Forece-Dsiplacement in {dir}', fontsize=14, pad=10)
        ax.set_xlabel('Displacement [mm]', fontsize=13, labelpad=8)
        ax.set_ylabel('Force [MPa]', fontsize=13, labelpad=8)
